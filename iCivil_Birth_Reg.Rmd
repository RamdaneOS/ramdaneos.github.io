...
##  5.0 La standardisation de la nommenclature du lieu de naissance) // Harmonizing the names of the place of birth by indicating only the name of the municipality where it took place
...
# 5.1 Le décompte des naissances par commune // Birth registration by municipality 
...
#Calcul du taux de déclaration des naissances par commune (on maintient uniquement les communes où il y a eu des declarations) // Computing the birth registration rate by municipality (only municipalities where births were declared to the civil registration center are kept in this analysis)
...
Sur un total de 18 lieux de naissance (ou communes), 15 apparaissent comme lieux où s'est effectué au moins une déclaration. Les trois communes où aucune déclaration n'a eu lieu sont: 
...
Out of a total of 18 places of birth (or commune), 15 appear as places where at least one declaration was done. There were three municipalities whith no declaration: 

* BOBO DIOULASSO
* OUAHIGOUYA
* SOAW

To understand the reason why no declaration was done in these three municipalities, it would be necessary to factor in the existence and closeness of civil registration centers equiped with iCivil in those regions. 

```{r "visualisation_declaration_per_location"} 
#Taux de declaration par commune // Declaration rate by municipality
...
## 5.2 Taux de declarations par sexe dans les communes // Rate of declaration by sexe, by municipality
...
We note that the lowest rate of birth declaration is in FADA, yet the municipality has the 3rd highest number of births. Also, in KAYA and NOUNA despite the fact that they have just received the iCivil system (2 months ago) show rather encouraging rates of declaration compared to that of OUAGADOUGOU which has had the system for 4 years. Declaration by sex is more or less the same in most cities. 
...
icivil_data_18_03_declared$temps_declaration <- as.integer(icivil_data_18_03_declared$temps_declaration) #conversion de la variable temps en une variable numerique // converting declaration time to integer 
...
However, as we know that the legal time limit for declaring a newborn in Burkina Faso is 2 months, this means that the declarations made in the UNFPA pilot sites where the iCivil technology has just been deployed (16-24 Jan) are certainly within the timeframe and can be considered as the real proportions of declarations in these settings, for the considered population. 
